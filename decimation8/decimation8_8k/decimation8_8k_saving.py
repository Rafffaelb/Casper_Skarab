...
p.add_option('-f', '--freq', dest='central_freq', type='int',default=1120,
help='Set the central frequency. default is 800MHz.')
p.add_option('-d', '--dec', dest='decimation', type='int',default=8,
help='Set the decimation. default is 8.')
...
def get_data():
	...
	a_1=struct.unpack('>4096L',skarabs[0].read('mem_left_0_0',4096*4,0))
	a_2=struct.unpack('>4096L',skarabs[0].read('mem_left_0_1',4096*4,0))
	#a_2 = a_1
...
if __name__ == '__main__':
	...
	(options, args) = p.parse_args()
        opts_fpgfile = 'decimation8_8k_2025-03-27_1610.fpg'
        opts_acc_len = 5722
        my_skarab_ip = '10.42.0.197'
...
try:
	...
	for i in range(skarab_adc_num):
        ...
        for i in range(skarab_num):
            ...
            skarabs[i].write_int('fft_shift', 4095)
	...
	aux_meusdados = []
	    ...
	    while True:
		    ...
		    if ((counter % 58) == 0):
		    	...
		    	aux_file_name = str(options.folder_out) + '/Dados_Fc%d_Dec%d_Ti%s_Tf%s_myaccn_%d' % (central_freq_MHZ, options.decimation, aux_inicio_str, aux_final_str, my_acc_n)
        	...
        	file_name = str(options.folder_out) + '/Dados_Fc%d_Dec%d_Ti%s_Tf%s' % (central_freq_MHZ, options.decimation, inicio_str, final_str)
